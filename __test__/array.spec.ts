...
describe('TArray', () => {
  ...
  describe('checks', () => {
    test('min', () => {
      ...
      expect(t.array(t.bigint()).min(0, { inclusive: false }).safeParse([]).ok).toBe(false)
    })

    test('max', () => {
      expect(t.array(t.boolean()).max(2).safeParse([true, false]).ok).toBe(true)
      expect(t.array(t.boolean()).max(2, { inclusive: false }).safeParse([true, false]).ok).toBe(false)
    })
    ...
    test('length', () => {
      ...
      expect(
        t
          .array(t.bigint())
          .length(2)
          .safeParse([BigInt(1)]).ok
      ).toBe(false)
      expect(t.array(t.never()).length(2).min(1))
    })
    ...
    test('compile-time errors', () => {
      ...
      // @ts-expect-error exclusive max 0 === -1
  ...
  test('inference', () => {
    ...
    assertEqual<
      ...
      [
        ...
        boolean?,
        boolean?,
        boolean?, // the rest is optional
        ...never[] // no more than 12
      ]
    >(true)
    assertEqual<
      t.infer<typeof bigintArrayWithFixedLength>,
      [bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, ...never[]]
    >(true)
